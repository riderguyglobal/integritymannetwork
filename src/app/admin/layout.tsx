...
export default function AdminLayout({
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-zinc-950">
      {/*  Mobile Top Bar  */}
      <div className="lg:hidden fixed top-0 left-0 right-0 z-50 h-14 bg-white/95 dark:bg-zinc-950/95 backdrop-blur-xl border-b border-zinc-200/50 dark:border-zinc-800/50 flex items-center justify-between px-4">
        <div className="flex items-center gap-3">
          <button
            onClick={() => setSidebarOpen(!sidebarOpen)}
            className="p-2 -ml-2 text-zinc-500 hover:text-zinc-900 dark:text-zinc-400 dark:hover:text-white transition-colors"
          >
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-bold text-zinc-900 dark:text-white">TIMN Admin</span>
          </div>
        </div>
      ...
      <aside
        className={cn(
          "fixed top-0 left-0 z-50 w-64 h-screen border-r border-zinc-200 dark:border-zinc-800/50 bg-white dark:bg-zinc-950 transition-transform duration-300 ease-in-out",
          "lg:translate-x-0",
          sidebarOpen ? "translate-x-0" : "-translate-x-full"
        )}
      >
        {/* Logo */}
        <div className="flex items-center gap-3 px-6 py-5 border-b border-zinc-200 dark:border-zinc-800/50">
          ...
          <div>
            <p className="font-display font-bold text-zinc-900 dark:text-white text-sm">TIMN</p>
              ...
              >
                <link.icon
                  className={cn(
                    ...
                    active ? "text-orange-500 dark:text-orange-400" : "text-zinc-400 dark:text-zinc-500 group-hover:text-orange-500 dark:group-hover:text-orange-400"
                  )}
                />
                {link.label}
                {active && (
                  <ChevronRight className="w-3 h-3 ml-auto text-orange-500/60 dark:text-orange-400/60" />
                )}
              </Link>
            );
          })}
